...
int
opt_iter(int x, int y, int x_0, int y_0)
{
        float dx_0 = (- x_0 / 2.f + x) / (WINDOW_WIDTH / 6.f);
        float dy_0 = ( y_0 / 2.f - y) / (WINDOW_HEIGHT / 6.f);
        float dx = dx_0;
        float dy = dy_0;
...
opt_set_pixels(int x_0, int y_0)
{
        ...
        for (int y = 0; y < WINDOW_HEIGHT; y++) {
                for (int x = 0; x < WINDOW_WIDTH; x++) {
                        pixels[x + WINDOW_WIDTH * y] = opt_iter(x, y, x_0, y_0);
                }
        }
